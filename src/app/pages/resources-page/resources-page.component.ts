...
import { createPanelGridConfig, PanelGridConfig } from '@/app/models/PanelGridConfig.model';

@Component({
  standalone: true,
  imports: [CommonModule, PanelGridComponent, PanelButtonComponent, VideoPlayerComponent],
  template: `
        <div class="w-full h-[calc(100vh-var(--header-height))] bg-gradient-to-br from-rich_black-500 to-prussian_blue-500 p-4 flex items-center justify-center">
            <app-panel-grid
                    class="h-full"
                    [config]="gridConfig"
                    [items]="gridConfig.buttons"
                    [customTemplate]="gridTemplate"
            />

            <ng-template #gridTemplate let-items="items">
                @for (button of items; track button.name) {
                        ...
                        >
                            @if (button.isVideo) {
                                <app-video-player
                                        ...
                                        [waitDurationBetweenVideoCycles]="gridConfig.waitDurationBetweenVideoCycles"
                                ></app-video-player>
                            }
                        </app-panel-button>
                    </div>
...
export class ResourcesPageComponent {
  gridConfig: PanelGridConfig = createPanelGridConfig({
    buttons: [
      {
        name: 'Characters',
        link: '/characters',
